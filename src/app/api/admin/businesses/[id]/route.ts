...
export async function PUT(
    ...
    if (action === 'approve') {
        const { error } = await supabase.from('businesses').update({ status: 'active', verified: true, updated_at: new Date().toISOString() }).eq('id', id);
        if (error) return NextResponse.json({ error: error.message }, { status: 500 });
        return NextResponse.json({ success: true, message: 'Business approved' });
    } else if (action === 'reject') {
        const { error } = await supabase.from('businesses').update({ status: 'rejected', status_note: reason ?? null, updated_at: new Date().toISOString() }).eq('id', id);
        if (error) return NextResponse.json({ error: error.message }, { status: 500 });
